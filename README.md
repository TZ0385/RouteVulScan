...
初次装载插件会在用户配置目录下生成Config_yaml.yaml配置文件，用来储存匹配规则。macOS/Linux 默认路径为 `~/.config/RouteVulScan/Config_yaml.yaml`。

默认规则已内置在插件中，首次运行时会自动初始化到本地配置文件。
...
* Head按钮，携带原始的请求头，默认开启

* Filter_Host 多行输入框，可以一行一个白名单host，*代表全部，如 *.baidu.com
...
* 2024-03-07 展示面板加入排序【✓】
* 2026-07-10 AI重构Maven化项目并更新依赖【✓】
* 2026-07-10 默认规则内置并迁移配置到用户目录【✓】
